...
function createCard(event) {
    ...
    eventCard.className = "card col-sm-12 col-md-6 col-lg-3"
    ...
    price.className = "card-text ms-2"
    details.appendChild(price)

    let buttonDetails = document.createElement("a")
    buttonDetails.href = "./details.html"
...
function paintCheckBox(categories, checkBoxContainer) {
...
function paintCards(events, cardContainer) {
...
function filterCategory(events) {
    ...
    if (categoriesChecked.length > 0) {
        let eventsFilter = []
        for (category of categoriesChecked) {
            eventsByCategory = events.filter(event => event.category.includes(category))
...
paintCheckBox(categoriesClean, checkBoxContainer)
paintCards(events, cardContainer)

let checkBoxes = document.querySelectorAll('input[type = "checkbox"]')
let btSearch = document.getElementById("btSearch")

checkBoxes.forEach(checkBox => {
    checkBox.addEventListener("change", (category) => {
        let eventsFilter = filterCategory(events)
        eventsFilter = filterText(eventsFilter)
        paintCards(eventsFilter, cardContainer)
    })
})

btSearch.addEventListener("click", () => {
    let eventsFilter = filterText(events)
    eventsFilter = filterCategory(eventsFilter)
    paintCards(eventsFilter, cardContainer)
})